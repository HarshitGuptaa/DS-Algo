...
vector<int> ngor(vector<int>&arr){
    ...
    for(int i=0;i<arr.size();i++){
        while(st.size()!=0 && arr[st.top()] < arr[i]){    //for next smaller on left replace with '>'
            ans[st.top()] = i;
            st.pop();
        }
        st.push(i);
...
vector<int> nextGreaterElements(vector<int>& arr) {
    ...
    for(int i=0;i<n*2;i++){
        while(st.size()!=0 && arr[st.top()] < arr[i%n]){  //for next smaller on left replace with '>'
            ans[st.top()] = arr[i%n];
            st.pop();
        }
        if(i<n) st.push(i);
...
//leet 32. Longest Valid Parentheses
